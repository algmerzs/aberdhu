...
const getUsers = async (prices) => {
    ...
    connection.query("SELECT * FROM users", (err, resu) => {
        ...
        // buscar criptomonedas seguidas por el usuario
        ...
        resu.forEach((e) => {
            ...
            connection.query(
                "SELECT symbol, current_price FROM indicators WHERE userId = ?",
                [id],
                async (err, resu) => {
                    if (err) throw err;

                    if (resu.length > 0) {
                        // guardar criptomonedas seguidas en array
...
// buscar precio actual en api y comparar con el guardado en BD
...
const updateDB = (price, symbol) => {
    connection.query(
        "UPDATE indicators SET current_price = ? WHERE symbol = ?",
        [price, symbol],
        (err, resu) => {
            if (err) throw err;
        }
...
// enviar correo con nuevo precio
